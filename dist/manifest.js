...
function normalizePortablePath(value, kind) {
    const normalized = value.trim();
    const segments = normalized.split("/");
    if (normalized.length === 0 ||
...
function normalizeRuntimeAccess(access) {
    if (!access)
        return undefined;
    const variables = deduplicate((access.variables ?? []).map((target) => normalizeTarget(target, false)), (target) => `${target.scope}\0${target.path}`);
    const resources = deduplicate((access.resources ?? []).map((target) => normalizeTarget(target, false)), (target) => `${target.scope}\0${target.path}`);
    const writeResources = deduplicate((access.writeResources ?? []).map((target) => normalizeTarget(target, true)), (target) => `${target.scope}\0${target.path}`);
    const writeVariables = deduplicate((access.writeVariables ?? []).map((target) => {
    ...
    if (total > 256)
        throw new Error("runtime access exceeds 256 paths");
    const result = {};
    if (variables.length > 0)
        result.variables = variables;
    if (resources.length > 0)
        result.resources = resources;
    if (writeVariables.length > 0)
        result.writeVariables = writeVariables;
    if (writeResources.length > 0)
        result.writeResources = writeResources;
...
function normalizeLabels(labels) {
    ...
    if (normalized.length > 16)
        throw new Error("at most 16 worker labels are allowed");
    return normalized.length > 0 ? normalized : undefined;
}
function schemaPath(source, actionName, kind, directory, files) {
    if (!source)
        return undefined;
    if (!("schema" in source))
        return normalizePortablePath(source.path, `${kind} schema path`);
    const safeName = actionName.replaceAll(/[^A-Za-z0-9_.-]/g, "_");
    const path = normalizePortablePath(source.path ?? `${directory}/${safeName}.${kind}.schema.json`, `${kind} schema path`);
...
export function resolveManifestFileName(options = {}) {
    const explicit = options.manifestFile?.trim();
    const configured = options.environment?.[MANIFEST_FILE_ENV]?.trim();
    return normalizePortablePath(explicit || configured || DEFAULT_MANIFEST_FILE, "manifest filename");
}
export function defineAction(options) {
    const name = validateActionName(options.name);
...
export function describeApp(app, options = {}) {
    const source = app;
    const schemaDirectory = normalizePortablePath(options.schemaDirectory ?? "schemas", "schema directory");
    const files = {};
    const actions = {};
    for (const action of source.actions) {
        const manifestAction = {};
        const inputSchema = schemaPath(action.inputSchema, action.name, "input", schemaDirectory, files);
        const outputSchema = schemaPath(action.outputSchema, action.name, "output", schemaDirectory, files);
        const operatorSettingsSchema = schemaPath(action.operatorSettingsSchema, action.name, "operator-settings", schemaDirectory, files);
        if (inputSchema)
            manifestAction.inputSchema = inputSchema;
        if (outputSchema)
            manifestAction.outputSchema = outputSchema;
        if (operatorSettingsSchema)
            manifestAction.operatorSettingsSchema = operatorSettingsSchema;
        if (action.timeout !== undefined)
            manifestAction.timeout = action.timeout;
        if (action.runsOn)
            manifestAction.runsOn = [...action.runsOn];
        if (action.runtimeAccess)
            manifestAction.runtimeAccess = action.runtimeAccess;
    ...
    if (source.options.timeout !== undefined)
        manifest.timeout = source.options.timeout;
    if (source.options.runsOn)
        manifest.runsOn = [...source.options.runsOn];
...
export function defineApp(options) {
    ...
    if (actions.length === 0)
        throw new Error("an App must define at least one action");
